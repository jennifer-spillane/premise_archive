...
#a script to pull out alignment files from a collection of them based on a provided list file
#this script is very similar to pull_alignments.py, but is using different files and therefore has different formatting
    ...
    try:
        with open("{0}".format(args.listfile), "r") as list_file:
            ...
            for item in os.scandir("{0}".format(args.aligndir)):
                just_name = re.match("Mus_musculus\|\d+_rename", "{0}".format(item.name))
                if just_name.group(0) in mouse_names:
