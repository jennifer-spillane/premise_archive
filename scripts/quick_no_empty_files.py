            ...
            with open("{0}".format(args.outfile), "w") as out_file:
                ...
                for line in aln_file:
                    ...
                    if stripped.startswith("charset"):
                        aln_range = re.match("(charset\s)[\/\w]*pruned39\/(Mus_musculus\|\d+_rename\s=\s)(\d+)\s-\s(\d+);", "{0}".format(stripped))
                        if aln_range:
                            aln_start = int(aln_range.group(3))
                            aln_end = int(aln_range.group(4))
                            #finding the alignment length and writing it with the name of the partition to the outfile
                            if aln_end > aln_start:
                                out_file.write("\t{0}{1}{2}-{3};\n".format(aln_range.group(1), aln_range.group(2), aln_range.group(3), aln_range.group(4)))
...
parser.add_argument("-a", "--alnfile", required = True, help = "path to concattenated alignment file (nexus)")
