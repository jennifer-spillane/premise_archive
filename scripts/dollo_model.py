...
def model(og):
    ...
    cur_id = multiprocessing.current_process().pid
    ...
    try:
        ...
        if og_name == "OG0001162":
            print("from the model function: OG0001162 - {0}".format(og), flush = True)
        if og_name == "OG0000145":
            print("from the model function: OG0000145 - {0}".format(og), flush = True)
        if og_name == "OG0000790":
            print("from the model function: OG0000790 - {0}".format(og), flush = True)
        if og_name == "OG0000067":
            print("from the model function: OG0000067 - {0}".format(og), flush = True)
        if og_name == "OG0001654":
            print("from the model function: OG0001654 - {0}".format(og), flush = True)
...
def writing(content):
    ...
    #if statements to try to detect the extra column of data in those two orthogroups
    if group_num == "OG0001162":
        print("from the writing function: OG0001162 - {0}".format(content), flush = True)
    if group_num == "OG0000145":
        print("from the writing function: OG0000145 - {0}".format(content), flush = True)
    if group_num == "OG0000790":
        print("from the writing function: OG0000790 - {0}".format(content), flush = True)
    if group_num == "OG0000067":
        print("from the writing function: OG0000067 - {0}".format(content), flush = True)
    if group_num == "OG0001654":
        print("from the writing function: OG0001654 - {0}".format(content), flush = True)
    ...
    # Debug check
    if group_num == "OG0000790":
        print("write func2: {}".format(presabs), flush=True)
    ...
    for item in presabs[1:]:
        # Debug check
        if group_num == "OG0000790":
            print("write func3: {}".format(item), flush=True)
    ...
    with lock:
        ...
        try:
            with open("{0}".format(args.output), write_mode) as final:
                # Debug check
                if group_num == "OG0000790":
                    print("write func4: {}".format(all_nodes))
	 	    ...
	 	    # Debug check
                    if group_num == "OG0000790":
                        print("write func5: {}".format(results_dict[og_num]))
...
first_time = multiprocessing.Value("i", 0)
